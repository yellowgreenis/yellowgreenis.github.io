...
$(document).ready(function() {
	...
	$("#fn_list").find("img").each(function(flii, fli) {
		$(fli).replaceWith("<a img='" + $(fli).attr("src") + "'>[Image]</a>");
	});
	$("#fn_list").find("a[img]").click(function() {
		location.href = $(this).attr("img");
	});

	function inimg() {
		$("#fnbox").find("a[img]").each(function(oai, oa) {
			$(oa).replaceWith("<img src='" + $(oa).attr("img") + "'/>");
	...
	$(".fn").hover(function() {
		if ( $(window).innerWidth() > "1340" ) {
			...
			if ( fn_top - $("#fnbox").outerHeight() + $(this).next().css("font-size").slice(0, -2)/10 >= document_top ) {
